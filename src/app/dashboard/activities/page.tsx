...
export default function ActivitiesPage() {
  ...
  const maxWeeklyCount = useMemo(() => Math.max(...weeklyData.map(d => d.activities), 1), [weeklyData]);
  const maxTypeCount = useMemo(() => Math.max(...actTypeData.map(d => d.count), 1), [actTypeData]);
          ...
          <div className="flex items-end gap-2 h-[140px]">
            {weeklyData.map((item) => (
              <div key={item.day} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
                <div
                  className="w-full max-w-[32px] bg-foreground/20 rounded-t-sm transition-all"
                  style={{ height: `${(item.activities / maxWeeklyCount) * 100}%` }}
                />
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-border">
          {recentActivities.map((activity) => {
            ...
            return (
              <div key={activity._id || activity.id} className="flex items-center gap-4 px-5 py-4 hover:bg-muted/20 transition-colors cursor-pointer">
